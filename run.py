...
def process_package(tox_env, pytest_version, name, version, description):
    ...
    if url:
        ...
        if response.status_code == 200:
            return PackageResult(name, version, 0, "SKIPPED", "Skipped", description, get_elapsed())
...
def post_test_results(test_results, tox_env, pytest_version, secret):
    ...
    if secret:
        ...
        data = {"secret": secret, "results": results}
            ...
            for package_result in chunk:
                if package_result.status != "SKIPPED":
                    test_results[(package_result.name, package_result.version)] = (
                        package_result.status_code,
                        package_result.output,
                        package_result.description,
                    )
            n_posted += post_test_results(
                test_results, tox_env=tox_env, pytest_version=pytest_version, secret=secret
            )
